...
`;
  ...
  const handleCreateScenario = async () => {
    setIsPlaying(true)
    setAiStatus('Senaryo Animasyonu Oluşturuluyor...')
    
    const code = `// AI Generated Scenario Code
const scene = await getScene();
...
const axes = new NumberPlane({ xRange: [-3, 7, 1], yRange: [-3, 7, 1], xLength: 10, yLength: 6, backgroundLineStyle: ${gridStyle} });
const graph = new FunctionGraph({ func: (x) => x*x - 4*x + 3, axes: axes, color: '#10b981' });

// 1. Eksenleri ve katsayı analizini çiz
scene.add(axes);
await scene.play(new Create(axes, { runTime: 1.5 }));

const coeffText = new Text({ weight: 'bold', text: "f(x) = x² - 4x + 3 (a = 1, b = -4, c = 3)", color: '#ffffff' });
coeffText.scale(0.6);
coeffText.moveTo([-1.5, 3.2, 0]);
await drawLabel(scene, coeffText);

// 2. Parabol grafiğini çiz
await scene.play(new Create(graph, { runTime: 2 }));

// 3. Tepe noktasını çiz
const pTepe = axes.c2p(2, -1);
const dotTepe = new Dot({ point: pTepe, color: '#f59e0b' });
const labelTepe = new Text({ weight: 'bold', text: "Tepe Noktası T(2, -1)", color: '#f59e0b' });
labelTepe.scale(0.4);
labelTepe.nextTo(dotTepe, [0, -1, 0], 0.3);
await drawLabel(scene, labelTepe, dotTepe);

// 4. Köklerin gösterimi
const pRoot1 = axes.c2p(1, 0);
const dotRoot1 = new Dot({ point: pRoot1, color: '#ef4444' });
const labelRoot1 = new Text({ weight: 'bold', text: "x₁ = 1", color: '#ef4444' });
labelRoot1.scale(0.4);
labelRoot1.nextTo(dotRoot1, [-0.5, 1, 0], 0.3);

const pRoot2 = axes.c2p(3, 0);
const dotRoot2 = new Dot({ point: pRoot2, color: '#ef4444' });
const labelRoot2 = new Text({ weight: 'bold', text: "x₂ = 3", color: '#ef4444' });
labelRoot2.scale(0.4);
labelRoot2.nextTo(dotRoot2, [0.5, 1, 0], 0.3);

await scene.play(new Create(dotRoot1), new Write(labelRoot1), new Create(dotRoot2), new Write(labelRoot2));

// 5. Simetri eksenini çiz
const symLine = new DashedLine({ start: axes.c2p(2, -3), end: axes.c2p(2, 6), color: '#a78bfa' });
const labelSym = new Text({ weight: 'bold', text: "Simetri Ekseni x = 2", color: '#a78bfa' });
labelSym.scale(0.4);
labelSym.moveTo([axes.c2p(2, 6)[0], axes.c2p(2, 6)[1] + 0.3, 0]);
await scene.play(new Create(symLine), new Write(labelSym));

// 6. y kesişimini çiz
const pY = axes.c2p(0, 3);
const dotY = new Dot({ point: pY, color: '#3b82f6' });
const labelY = new Text({ weight: 'bold', text: "y-kesişimi (0, 3)", color: '#3b82f6' });
labelY.scale(0.4);
labelY.nextTo(dotY, [-1, 0, 0], 0.3);
await drawLabel(scene, labelY, dotY);
`;
    setEditorCode(code)
    
    await new Promise((res) => setTimeout(res, 300))
    setAiStatus('Animasyon İşleniyor...')
    addLog('success', `Öğretmen senaryosu kodu oluşturuldu. Render başlatılıyor...`)
